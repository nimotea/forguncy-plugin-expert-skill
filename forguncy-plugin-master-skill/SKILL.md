...
### 3. 生成设计计划 (必选)
- **动作**：在正式编写代码前，必须先生成一份设计计划 Markdown 文件（例如 `DESIGN.md`）。
- **内容要求**：
    1.  **功能概述**：简述插件要实现的功能。
    2.  **拟用模板**：明确将使用哪个 `templates/` 下的文件。
    3.  **引用规范**：列出将参考的 `docs/` 文档路径（例如：`docs/references/ServerCommand/Add_Property_String.md`）。
    4.  **属性设计**：列出计划添加的属性及其特性（如 `[DisplayName]`, `[FormulaProperty]`）。
    5.  **核心逻辑**：简述 `Execute` 或前端 JS 的实现思路。
- **执行**：
    - 将设计文档写入项目目录（如果项目尚未创建，则直接在回复中展示）。
    - **暂停**：明确告知用户：“请确认设计计划无误。确认后我将开始生成代码。”
    - **严禁**：在用户确认计划前生成最终代码。
