...
By default, logs are written to `feedback_logs.jsonl` in the workspace root.
You can override this by passing the `--log-file` argument to the script.

```bash
python .trae/skills/dropQ/scripts/log_feedback.py '<json_string>' --log-file 'my_custom_log.jsonl'
```
...
3.  **Persist**: Run the `log_feedback.py` script to append the data to the log file.
4.  **Confirm**: Notify the user that the feedback has been recorded.
...
Execute the logging script with the JSON string.
...
```bash
python .trae/skills/dropQ/scripts/log_feedback.py '{"title": "Simple Test"}'
```

**Option B: Using a temporary file (Recommended for stability)**
1. Write the JSON content to a temporary file (e.g., `temp_feedback.json`).
2. Run the script with the file path:
```bash
python .trae/skills/dropQ/scripts/log_feedback.py temp_feedback.json
```
